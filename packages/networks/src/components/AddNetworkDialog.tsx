...
import { useExtensionEvent, useLocalDB, fireExtensionEvent } from '@yakapa/shared';
...
export const AddNetworkDialog: FC<Props> = props => {
  ...
  useExtensionEvent(Events.OpenAddNetworkDialog, () => setOpen(true));
  ...
  const submit = async (values: FormValues, actions: FormikActions<FormValues>) => {
    try {
      const network = await prisma.createNetwork({ name: values.networkName, master: { connect: { id: agentId } } });
      fireExtensionEvent(Events.SelectNetwork, { id: network.id });
    } catch (e) {
